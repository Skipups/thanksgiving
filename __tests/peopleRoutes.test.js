  ...
  describe("GET to /api/people", () => {
    ...
    it("should return users and their Dishes using `include_dishes=true` query string", async () => {
      try {
        ...
        // response
        const include_dishes = await request(app).get(
          "/api/people/?include_dishes=true"
        );
        //test assertion
        expect(include_dishes.statusCode).toBe(200);
        expect(include_dishes.headers["content-type"]).toEqual(
          expect.stringContaining("json")
        );

        const includingDishes = include_dishes.body;
        expect(includingDishes.length).toBe(3);
        expect(includingDishes).toEqual(
          expect.arrayContaining([
            expect.objectContaining(person1),
            expect.objectContaining(person3)
          ])
    ...
    it("should create a new person and return that persons information if all the required information is given", async () => {
      await request(app)
        ...
        .then(response => {
          ...
          expect(person).toEqual(expect.objectContaining(person1));
        });
    ...
    it("should update a persons information", async () => {
      ...
      await request(app)
        ...
        .then(response => {
          ...
          //expect(updatedPerson).toEqual(expect.objectContaining(person1));
        });
    });
